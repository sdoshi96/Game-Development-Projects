...
def draw(canvas):
    ...
    if ball_pos[0] <= (PAD_WIDTH + BALL_RADIUS):
        if ball_pos[1] < (paddle1_pos - HALF_PAD_HEIGHT) or ball_pos[1] > (paddle1_pos + HALF_PAD_HEIGHT):
            spawn_ball(True)
            score2 += 1
        elif (paddle1_pos - HALF_PAD_HEIGHT) <= ball_pos[1] and ball_pos[1] <= (paddle1_pos + HALF_PAD_HEIGHT):
    ...
    if  ball_pos[0] >= (WIDTH - PAD_WIDTH - BALL_RADIUS):
        if ball_pos[1] < (paddle2_pos - HALF_PAD_HEIGHT) or ball_pos[1] > (paddle2_pos + HALF_PAD_HEIGHT):
            spawn_ball(False)
            score1 += 1
        elif (paddle2_pos - HALF_PAD_HEIGHT) <= ball_pos[1] and ball_pos[1] <= (paddle2_pos + HALF_PAD_HEIGHT):
            ball_vel[0] = -ball_vel[0] * 1.1
    ...
    canvas.draw_line([WIDTH, paddle2_pos + HALF_PAD_HEIGHT], [WIDTH,paddle2_pos - HALF_PAD_HEIGHT], 18, "Green") 
    
    # draw scores
    canvas.draw_text(str(score1), [135,50], 50, "Red")
    canvas.draw_text(str(score2), [440, 50], 50, "Red")    
